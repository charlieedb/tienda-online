...
import { getCartItemPricing, getCartItemUnits, getRemainingStock, useCartStore } from "@/store/cart";
...
export function CartView({ onContinue, onRequireAuth }: { onContinue: () => void; onRequireAuth?: () => void }) {
  ...
  return <>
    {sent ? <section className="order-success"><div className="success-check"><Icon name="check"/></div><h2>Pedido confirmado :)</h2><p>Nos comunicaremos con vos en breve, para coordinar la entrega y la forma de pago.<br/>Muchas gracias</p>{sentWarning ? <div className="checkout-error" role="status">{sentWarning}</div> : null}<button type="button" className="primary-action" onClick={() => window.location.reload()}>Volver al inicio</button></section> : <section className="cart-page">
      ...
      <div className="cart-list"><AnimatePresence initial={false}>{items.map((item) => { const remaining = getRemainingStock(items, item.productId, item.stockLimit); const canAdd = remaining === undefined || remaining >= getCartItemUnits({ ...item, qty: 1 }); const pricing = getCartItemPricing(item); const couponEligibleSubtotal = Number(appliedCode?.eligibleSubtotalByItem?.[item.id] || 0); const couponApplies = couponEligibleSubtotal > 0; const looseDiscountedTotal = couponEligibleSubtotal * (1 - Number(appliedCode?.percentage || 0) / 100); return <motion.article layout exit={{ opacity: 0, x: 24 }} key={item.id} className={`cart-item ${couponApplies ? "has-coupon-discount" : ""}`}>
        <div className="cart-item-copy"><strong>{item.name}</strong><span>{item.label}</span><div className="cart-item-price-split">{pricing.promoUnits > 0 ? <div className="cart-item-price-row is-promo"><span>Promo caja · {pricing.promoUnits} unid.</span><b>{money.format(pricing.promoUnits * Number(item.promoPackUnitPrice || 0))}</b></div> : null}{pricing.regularUnits > 0 && pricing.promoUnits > 0 ? <div className={`cart-item-price-row ${couponApplies ? "is-coupon" : ""}`}><span>Unidades sueltas · {pricing.regularUnits} unid.</span><span>{couponApplies ? <><del>{money.format(couponEligibleSubtotal)}</del><b>{money.format(looseDiscountedTotal)}</b></> : <b>{money.format(pricing.regularUnits * item.price)}</b>}</span></div> : null}{pricing.promoUnits === 0 ? (couponApplies ? <div className="cart-item-coupon-price"><del>{money.format(couponEligibleSubtotal)}</del><b>{money.format(looseDiscountedTotal)}</b><small>Cupón {appliedCode?.percentage}% aplicado</small></div> : <><b>{money.format(pricing.total)}</b>{appliedCode ? <small className="cart-item-promo-note">Ya tiene promoción, el cupón no aplica</small> : null}</>) : null}</div></div>
        <div className="cart-item-actions"><div className="stepper compact"><button type="button" onClick={() => decItem(item.id)} aria-label={`Disminuir ${item.name}`}><Icon name="minus" /></button><output>{item.qty}</output><button type="button" onClick={() => addItem({ id: item.id, productId: item.productId, name: item.name, variant: item.variant, label: item.label, price: item.price, listPrice: item.listPrice, discountPct: item.discountPct, unitPriceFinal: item.unitPriceFinal, unitsPerPack: item.unitsPerPack, stockLimit: item.stockLimit, promoPackQty: item.promoPackQty, promoPackUnitPrice: item.promoPackUnitPrice }, 1)} aria-label={`Aumentar ${item.name}`} disabled={!canAdd}><Icon name="plus" /></button></div><button className="remove-button" type="button" onClick={() => removeItem(item.id)}>Quitar</button></div>
