...
export function CartView({ onContinue, onRequireAuth }: { onContinue: () => void; onRequireAuth?: () => void }) {
  ...
  return <>
    {sent ? <section className="order-success"><div className="success-check"><Icon name="check"/></div><h2>Pedido confirmado :)</h2><p>Nos comunicaremos con vos en breve, para coordinar la entrega y la forma de pago.<br/>Muchas gracias</p>{sentWarning ? <div className="checkout-error" role="status">{sentWarning}</div> : null}<button type="button" className="primary-action" onClick={() => window.location.reload()}>Volver al inicio</button></section> : <section className="cart-page">
      ...
      <div className="cart-list"><AnimatePresence initial={false}>{items.map((item) => { const remaining = getRemainingStock(items, item.productId, item.stockLimit); const canAdd = remaining === undefined || remaining >= getCartItemUnits({ ...item, qty: 1 }); const couponApplies = Boolean(appliedCode?.eligibleItemIds.includes(item.id)); const originalLineTotal = item.price * item.qty; const discountedLineTotal = couponApplies ? originalLineTotal * (1 - Number(appliedCode?.percentage || 0) / 100) : originalLineTotal; return <motion.article layout exit={{ opacity: 0, x: 24 }} key={item.id} className={`cart-item ${couponApplies ? "has-coupon-discount" : ""}`}>
        <div className="cart-item-copy"><strong>{item.name}</strong><span>{item.label}</span>{couponApplies ? <div className="cart-item-coupon-price"><del>{money.format(originalLineTotal)}</del><b>{money.format(discountedLineTotal)}</b><small>Cupón {appliedCode?.percentage}% aplicado</small></div> : <><b>{money.format(originalLineTotal)}</b>{appliedCode ? <small className="cart-item-promo-note">Ya tiene promoción, el cupón no aplica</small> : null}</>}</div>
