  ...
  return <>
    {sent ? <section className="order-success"><div className="success-check"><Icon name="check"/></div><h2>Pedido confirmado :)</h2><p>Nos comunicaremos con vos en breve, para coordinar la entrega y la forma de pago.<br/>Muchas gracias</p>{sentWarning ? <div className="checkout-error" role="status">{sentWarning}</div> : null}<button type="button" className="primary-action" onClick={() => window.location.reload()}>Volver al inicio</button></section> : <section className="cart-page">
      ...
      <div className="cart-list"><AnimatePresence initial={false}>{items.map((item) => { const remaining = getRemainingStock(items, item.productId, item.stockLimit); const canAdd = remaining === undefined || remaining >= getCartItemUnits({ ...item, qty: 1 }); const pricing = pricingByItem.get(item.id)!; const couponEligibleSubtotal = Number(appliedCode?.eligibleSubtotalByItem?.[item.id] || 0); const couponApplies = couponEligibleSubtotal > 0; const looseDiscountedTotal = couponEligibleSubtotal * (1 - Number(appliedCode?.percentage || 0) / 100); return <motion.article layout exit={{ opacity: 0, x: 24 }} key={item.id} className={`cart-item ${couponApplies ? "has-coupon-discount" : ""}`}>
        <div className="cart-item-copy"><strong>{item.name}</strong><span>{item.label}</span><div className="cart-item-price-split">{pricing.promoUnits > 0 ? <div className="cart-item-price-row is-promo"><span>Con oferta · {pricing.promoUnits} unid.</span><b>{money.format(pricing.promoSubtotal)}</b></div> : null}{pricing.regularUnits > 0 && pricing.promoUnits > 0 ? <div className={`cart-item-price-row ${couponApplies ? "is-coupon" : ""}`}><span>A precio normal · {pricing.regularUnits} unid.</span><span>{couponApplies ? <><del>{money.format(couponEligibleSubtotal)}</del><b>{money.format(looseDiscountedTotal)}</b></> : <b>{money.format(pricing.regularSubtotal)}</b>}</span></div> : null}{pricing.promoUnits === 0 ? (couponApplies ? <div className="cart-item-coupon-price"><del>{money.format(couponEligibleSubtotal)}</del><b>{money.format(looseDiscountedTotal)}</b><small>Cupón {appliedCode?.percentage}% aplicado</small></div> : <><b>{money.format(pricing.total)}</b>{appliedCode ? <small className="cart-item-promo-note">Ya tiene promoción, el cupón no aplica</small> : null}</>) : null}{Number(item.offerMaxUnits || 0) > 0 && Number(item.offerUsedUnits || 0) >= Number(item.offerMaxUnits || 0) ? <small className="cart-item-promo-note">Ya usaste el cupo de esta oferta hoy. Se cobra a precio normal.</small> : null}</div></div>
