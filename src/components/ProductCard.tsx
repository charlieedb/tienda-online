...
function ProductCardInner({ product, eager = false, linkImageToDetail = true, detailCompact = false, onPriceChange }: { product: Product; eager?: boolean; linkImageToDetail?: boolean; detailCompact?: boolean; onPriceChange?: (price: number) => void }) {
  ...
  return <motion.article ref={cardRef} onClickCapture={() => { if (sponsorContext) trackPromotionClick(sponsorContext); }} className={`product-card ${detailCompact ? "is-detail-compact" : ""} ${!product.active ? "is-unavailable" : ""} ${sponsor ? "is-sponsored" : ""}`} initial={false} animate={reduceMotion ? undefined : { opacity: 1, y: 0 }} transition={{ duration: 0.22 }}>
