      ...
      <div className={`desktop-layout ${tab === "cart" ? "is-cart-view" : ""} ${tab === "business" ? "is-business-view" : ""}`}>
        ...
        <main id="main-content" className={itemCount ? "has-mini-cart" : ""}>
          <AnimatePresence mode="popLayout" initial={false}>
            ...
            {tab === "coupons" ? <motion.div className="view" key="coupons" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }}><MyCouponsPage onLogin={() => onRequestLogin("login")} onUse={(code) => { window.sessionStorage.setItem("joma.pendingCoupon", code); window.dispatchEvent(new CustomEvent("joma:coupon-selected", { detail: code })); goTo("cart"); }} /></motion.div> : null}
            {tab === "info" ? (
              <motion.div
                className="view"
                key={`info-${infoPage}`}
